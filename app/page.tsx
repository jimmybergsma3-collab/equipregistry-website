...
function getStatus(serial: string): Status {
  ...
  if (s === "ER-REG-001") {
    return {
      ...
      actions: [
        { label: "View passport", href: "/action?type=passport", style: "primary" },
        { label: "Contact insurer", href: "/action?type=verify", style: "secondary" },
      ],
    };
  }
  ...
  if (s === "ER-HIS-404") {
    return {
      ...
      actions: [
        { label: "Request verification", href: "/action?type=verify", style: "primary" },
        { label: "Register documents", href: "/action?type=register", style: "secondary" },
      ],
    };
  }
  ...
  if (s === "ER-STOL-777") {
    return {
      ...
      actions: [
        { label: "Report sighting", href: "/action?type=report", style: "primary" },
        { label: "Contact authorities", href: "/action?type=report", style: "danger" },
        { label: "Verify case ID", href: "/action?type=verify", style: "secondary" },
      ],
    };
  }
  ...
  return {
    ...
    actions: [{ label: "Register this machine", href: "/action?type=register", style: "primary" }],
  };
}
...
export default async function Home({ searchParams }: Props) {
  ...
  const dashboardTargetFor = (registryId?: string) =>
    registryId ? `/dashboard/passport/${encodeURIComponent(registryId)}` : "/dashboard";

  const resolveActionHref = (href: string) => {
    // Keep public actions public
    if (!href.startsWith("/action")) return href;

    if (!isLoggedIn) return href; // /action already sends you to login with next

    // If user is logged in, skip action gate and go straight to dashboard
    // For flows that relate to a serial, prefer dashboard passport
